...
def acquire_work(session, worker, job_set_uuid = None, max_hired = 1):
    ...
    if job_set_uuid is None:
        ...
        job_filter =                                         \
            ...
            & (JobRecord.job_set_uuid == job_set_uuid)
...
def main_loop(session, job_set_uuid = None, worker_uuid = None, max_hired = 1):
    """
...
def main(
    ...
    with SQL_Engines.default as engines:
        with engines.make_session(url)() as session:
            main_loop(session, job_set_uuid, worker_uuid, max_hired)
